...
('int',       default, help, min,max) # (min==None is no minimum, and
                                         the same for max)
...
options = (
  ('ui',
    (
      ('alwaysShowFeed', ('bool', False,
            "Always show a link to the post's feed when displaying a post")),
      ('hideOnTrayClick', ('bool', True,
            "Hide the main window when clicking on the tray icon")),
      ('startOnTray', ('bool', False,
            "Don't show the main window on startup")),
      ('feedOnTitle', ('bool', False,
            "Add the feed name to the item title (Example: 'My Blog: "\
            "what I ate this morning'")),
    )
  ),
  ('options',
    (
      ('defaultRefresh'      , ('int', 1800,
        "How often feeds should be refreshed by default (in seconds).",
        300, None )),
      ('maxPostsDisplayed'   , ('int', 1000,
        "Limit the display to this many posts."\
        "If set too high, opening 'All Feeds' may take forever", 0, None)),
      ('defaultExpiration'   , ('int', 7, "How long should articles "\
                                 "be kept by default. (In days). You need"\
                                 " to restart uRSSus to have an effect.",
                                 0, 9999)),
      ('fetchOnStartup'      , ('bool', False,
                                 "Fetch all feeds on startup.")),
      ('showDebugDialog'     , ('bool', False, "Show dialog on uncatched "\
                                "exceptions. Only a good idea if you want "\
                                "to help debug uRSSus ;-). You need to "\
                                "restart uRSSus to have an effect.")),
      ('followLinksInUrssus' , ('bool', False, "If True, links in articles"\
                                " will open in uRSSus window, instead of in "\
                                "your web browser. You can still open in the"\
                                " browser by using right click and 'Open in "\
                                "New Window'")),
    )
  ),
)

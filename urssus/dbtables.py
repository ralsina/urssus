...
class Feed(elixir.Entity):
    ...
    children       = elixir.OneToMany('Feed', inverse='parent',
                                      order_by='text', cascade="delete")
    parent         = elixir.ManyToOne('Feed')
    posts          = elixir.OneToMany('Post', order_by="-date",
                                      inverse='feed',
                                      cascade="delete,delete-orphan")
    lastUpdated    = elixir.Field(elixir.DateTime,
    ...
    # updateInterval -1 means use the app default, any other value,
    # it's in minutes
    ...
    # Added in schema version 7
    lastModified   = elixir.Field(elixir.DateTime,
    ...
    def getChildren(self, order_by='-text'):
        ...
        if order_by=='text':
            _cmp=lambda x, y: mul* cmp(normalizar(x.text.lower()),
                                       normalizar(y.text.lower()))
    ...
    def markAsRead(self):
        if self.xmlUrl: # regular feed
            try:
                Post.table.update().where(Post.unread==True).\
                    where(Post.feed==self).\
                    values(unread=False).execute()
    ...
    def allPostsQuery(self):
        '''Returns a query that should give you all the posts
        contained in this folder's children down to any level.
        Required for aggregate feeds, I'm afraid'''
        af=[feed.id for feed in self.allFeeds()]
        return Post.query.filter(Post.table._columns.get('feed_id').in_(af))

    def allPosts(self):
        '''This is used if you want all posts in this feed
        as well as in its childrens.
    ...
    def lastChild(self, order_by='-text'):
        '''Goes to the last possible child of this feed
    ...
    def previousUnreadFeed(self, order_by='-text'):
        ...
        if not self.parent: # At root feed
            return self.lastChild(order_by=order_by).\
                previousUnreadFeed(order_by=order_by)
        sibs=self.parent.getChildren(order_by=order_by)
        ...
        if self.parent and self.parent.unreadCount():
            if self.parent.xmlUrl:
                ...
                return self.parent.lastChild(order_by=order_by).\
                    previousUnreadFeed(order_by=order_by)
        elif self.parent:
    ...
    def nextUnreadFeed(self, order_by='-text'):
        ...
        if not self.parent:
            # We are the root feed, and have no children unread:
            # there's no unread
            return None
    ...
    def unreadCount(self):
        if self.getChildren():
            self.curUnread=sum([f.unreadCount() \
    ...
    def updateFeedData(self, parsedFeed):
        ...
        if not self.htmlUrl:
            if 'link' in d['feed']:
                ...
                # This happens, for instance, on deleted
                # blogger blogs (everyone's clever)
                self.htmlUrl=None
